...
//    csr_graph.DisplayGraph();
  ...
  {
    std::cerr << "Usage: ./SSSP market_graph_file src output_file undirected(0 or 1) device max_queue_sizing" << std::endl;
    exit(1);
  }
  ...
  Value* h_dists = (Value*) malloc(sizeof(Value) * csr_graph.nodes);
