...
//				delete[] test_vid;
            ...
            std::cout << "Contract time took: " << elapsedcontract * 1000 << " ms" << std::endl;
            std::cout << "Expand time took: " << elapsedexpand * 1000 << " ms" << std::endl;
            // Compute nodes and edges visited
            SizeT edges_visited = 0;
            SizeT nodes_visited = 0;
            VertexId* h_dists = new VertexId[graph_slice->nodes];
            cudaMemcpy(h_dists, graph_slice->d_labels, graph_slice->nodes * sizeof (VertexId), cudaMemcpyDeviceToHost);
