      ...
      namespace vertex_centric
      {
        namespace scatter
        {
          ...
          struct Cta
          {
              ...
              struct Iterate
              {
                ...
                static __device__ __forceinline__ void BitmaskCull(Cta *cta, Tile *tile)
                {
                  if (tile->vertex_id[LOAD][VEC] != -1)
                  {
                    ...
                    VisitedMask tex_mask_byte = tex1Dfetch(BitmaskTex<VisitedMask>::ref, mask_byte_offset);
                    ...
                    else
                    {
                      ...
                      util::io::ModifiedLoad<util::io::ld::cg>::Ld(mask_byte, cta->d_visited_mask + mask_byte_offset);
                      ...
                      else
                      {
                        ...
                        mask_byte |= mask_bit;
                        util::io::ModifiedStore<util::io::st::cg>::St(mask_byte, cta->d_visited_mask + mask_byte_offset);
                      }
                    }
                  }
                ...
                {
                  //printf("tile->vertex_id[LOAD][VEC]=%d\n", tile->vertex_id[LOAD][VEC]);
                  if (tile->vertex_id[LOAD][VEC] != -1)
                  {
                    ...
                    // Load label of node
                    VertexId label;
                    util::io::ModifiedLoad<util::io::ld::cg>::Ld(label, cta->d_labels + row_id);
                    printf("row_id=%d, label=%d\n", row_id, label);
                    ...
                    if (label != -1)
                    {
                      ...
                      {
                        printf("iteration=%d, row_id=%d, d_sigmas[row_id]=%d, LOAD=%d, VEC=%d, tile->predecessor_id[LOAD][VEC]=%d\n",
                            cta->iteration, row_id, cta->d_sigmas[row_id], LOAD, VEC, tile->predecessor_id[LOAD][VEC]);
                    ...
                    else
                    {
                      ...
                      util::io::ModifiedStore<util::io::st::cg>::St(cta->iteration, cta->d_labels + row_id);

                      // Update predecessor with predecessor value
                      util::io::ModifiedStore<util::io::st::cg>::St(tile->predecessor_id[LOAD][VEC], cta->d_preds + row_id);
            ...
            Cta(VertexId iteration, VertexId queue_index, int num_gpus, SmemStorage &smem_storage, VertexId *d_in, VertexId *d_out, VertexId *d_predecessor_in, VertexId *d_labels, VertexId *d_preds,
                EValue *d_sigmas, VisitedMask *d_visited_mask, util::CtaWorkProgress &work_progress, SizeT max_vertex_frontier) :

                iteration(iteration), queue_index(queue_index), num_gpus(num_gpus), raking_details(smem_storage.state.raking_elements, smem_storage.state.warpscan, 0), smem_storage(smem_storage), d_in(
                    d_in), d_out(d_out), d_predecessor_in(d_predecessor_in), d_labels(d_labels), d_preds(d_preds), d_sigmas(d_sigmas), d_visited_mask(d_visited_mask), work_progress(work_progress), max_vertex_frontier(
                    max_vertex_frontier), bitmask_cull((KernelPolicy::END_BITMASK_CULL < 0) ? true : // always bitmask cull
                                                       (KernelPolicy::END_BITMASK_CULL == 0) ? false : // never bitmask cull
             ...
             */
            __device__ __forceinline__ void ProcessTile(SizeT cta_offset, const SizeT &guarded_elements = KernelPolicy::TILE_ELEMENTS)
            {
              ...
              util::io::LoadTile<KernelPolicy::LOG_LOADS_PER_TILE, KernelPolicy::LOG_LOAD_VEC_SIZE, KernelPolicy::THREADS, KernelPolicy::QUEUE_READ_MODIFIER, false>::LoadValid(tile.predecessor_id,
                  d_predecessor_in, cta_offset, guarded_elements);
              ...
              SizeT new_queue_offset = util::scan::CooperativeTileScan<KernelPolicy::LOAD_VEC_SIZE>::ScanTileWithEnqueue(raking_details, tile.ranks,
              ...
              // on flags
              util::io::ScatterTile<KernelPolicy::LOG_LOADS_PER_TILE, KernelPolicy::LOG_LOAD_VEC_SIZE, KernelPolicy::THREADS, KernelPolicy::QUEUE_WRITE_MODIFIER>::Scatter(d_out, tile.vertex_id,
