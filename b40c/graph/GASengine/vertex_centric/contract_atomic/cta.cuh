              ...
              struct Iterate
              {
                ...
                static __device__ __forceinline__ void VertexCull(Cta *cta, Tile *tile)
                {
                  if (tile->vertex_id[LOAD][VEC] != -1)
                  {
                    ...
                    VertexId row_id = tile->vertex_id[LOAD][VEC]; //(tile->vertex_id[LOAD][VEC] & KernelPolicy::VERTEX_ID_MASK) / cta->num_gpus;
                    atomicMin(&cta->d_gather_results[row_id], tile->predecessor_id[LOAD][VEC]);
...
//					}
             ...
             * Constructor
             */
            __device__ __forceinline__ Cta(VertexId iteration, VertexId queue_index, int num_gpus, SmemStorage &smem_storage, VertexId *d_in, VertexId *d_out, VertexId *d_predecessor_in,
                EValue *d_gather_results,
//		VertexId 				*d_labels,
//		VertexId                *d_preds,
//        EValue                  *d_sigmas,
                VisitedMask *d_visited_mask, util::CtaWorkProgress &work_progress, SizeT max_vertex_frontier) :
                ...
                iteration(iteration), queue_index(queue_index), num_gpus(num_gpus), raking_details(smem_storage.state.raking_elements, smem_storage.state.warpscan, 0), smem_storage(smem_storage), d_in(
                    d_in), d_out(d_out), d_predecessor_in(d_predecessor_in), d_gather_results(d_gather_results),
//			d_labels(d_labels),
//			d_preds(d_preds),
//            d_sigmas(d_sigmas),
                d_visited_mask(d_visited_mask), work_progress(work_progress), max_vertex_frontier(max_vertex_frontier), bitmask_cull((KernelPolicy::END_BITMASK_CULL < 0) ? true : // always bitmask cull
