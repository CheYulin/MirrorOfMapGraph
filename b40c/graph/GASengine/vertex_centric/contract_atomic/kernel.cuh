...
#include <b40c/graph/GASengine/vertex_centric/contract_atomic/cta.cuh>
        ...
        namespace contract_atomic
        {
          ...
          struct SweepPass
          {
            static __device__ __forceinline__ void Invoke(typename KernelPolicy::VertexId &iteration, typename KernelPolicy::VertexId &queue_index, typename KernelPolicy::VertexId &steal_index,
                ...
                typename KernelPolicy::EValue *&d_gather_results,
                ...
                typename KernelPolicy::SmemStorage &smem_storage)
            {
              ...
              Cta cta(iteration, queue_index, num_gpus, smem_storage, d_edge_frontier, d_vertex_frontier, d_predecessor, d_gather_results, d_visited_mask, work_progress,
                  max_vertex_frontier);
          ...
          struct Dispatch
          {
            ...
            static __device__ __forceinline__ void Kernel(VertexId &src, VertexId &iteration, SizeT &num_elements, VertexId &queue_index, VertexId &steal_index, int &num_gpus, volatile int *&d_done,
                VertexId *&d_edge_frontier, VertexId *&d_vertex_frontier, VertexId *&d_predecessor, EValue *&d_dists, EValue *&d_dists_out, EValue *&d_gather_results, int *&d_changed,
          ...
          struct Dispatch<KernelPolicy, true>
          {
            ...
            static __device__ __forceinline__ void Kernel(VertexId &src, VertexId &iteration, SizeT &num_elements, VertexId &queue_index, VertexId &steal_index, int &num_gpus, volatile int *&d_done,
                VertexId *&d_edge_frontier, VertexId *&d_vertex_frontier,
                VertexId                  *&d_predecessor,
                EValue *&d_dists, EValue *&d_dists_out, EValue *&d_gather_results, int *&d_changed,
                VisitedMask *&d_visited_mask, util::CtaWorkProgress &work_progress, SizeT &max_edge_frontier, SizeT &max_vertex_frontier, util::KernelRuntimeStats &kernel_stats)
            {
...
//
//                      typename KernelPolicy::EValue init_sigma = 1;
//                      util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(init_sigma, d_sigmas + src);

                      int init_dist = 0;
                      util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(init_dist, d_dists + src);
                      util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(init_dist, d_dists_out + src);
                ...
                // across CTAs
                SweepPass<KernelPolicy, false>::Invoke(iteration, queue_index, steal_index, num_gpus, d_edge_frontier, d_vertex_frontier, d_predecessor, d_gather_results, d_visited_mask,
...
//                  printf("Contract_kernel259: num_elements = %d\n", num_elements);
                ...
                SweepPass<KernelPolicy, KernelPolicy::WORK_STEALING>::Invoke(iteration, queue_index, steal_index, num_gpus, d_edge_frontier, d_vertex_frontier, d_predecessor, d_gather_results,
...
//              typename KernelPolicy::EValue *d_sigmas,                  // BFS sigmas to set
              typename KernelPolicy::EValue *d_dists,                        //distances
              typename KernelPolicy::EValue *d_dists_out,                        //distances
              typename KernelPolicy::EValue *d_gather_results,                        //distances
          ...
          {
            Dispatch<KernelPolicy>::Kernel(src, iteration, num_elements, queue_index, steal_index, num_gpus, d_done, d_edge_frontier, d_vertex_frontier, d_predecessor,
                                           d_dists, d_dists_out, d_gather_results, d_changed,
