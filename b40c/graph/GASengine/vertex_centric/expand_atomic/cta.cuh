        ...
        namespace expand_atomic
        {
          ...
          struct Cta
          {
            ...
            static __device__                                        __forceinline__ T WarpReduce(T* partial, size_t warp_id)
            {
              ...
              {
                if (threadIdx.x < warp_id * B40C_WARP_THREADS_BFS(KernelPolicy::CUDA_ARCH) + s) partial[threadIdx.x] += partial[threadIdx.x + s];
                __syncthreads();
              }
            ...
            int *d_dists;
            int *d_dists_out;
            EValue *d_gather_results;
            int *d_changed;                      //changed
            int *d_visited_flag; //
...
//                    if(cta->iteration == 2)
//                    printf("Inspect219: bidx=%d, tidx=%d, row_id=%d, row_range.x=%d, row_range.y=%d\n", blockIdx.x, threadIdx.x, row_id, row_range.x, row_range.y);
                ...
                static __device__ __forceinline__ void ExpandByCta(Cta *cta, Tile *tile)
                {
                  ...
                  {
//                    int allchanged;
//                    util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(allchanged, cta->d_changed + tile->vertex_id[LOAD][VEC]);
                    // Vie
...
//                      util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(allchanged - 2, cta->d_changed + tile->vertex_id[LOAD][VEC]);
...
//                    VertexId predecessor_id = cta->smem_storage.state.warp_comm[0][3];
                    int changed;	// = cta->smem_storage.state.warp_comm[0][3];
                    util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(changed, cta->d_changed + cta->smem_storage.state.warp_comm[0][3]);
                    EValue dist;
                    util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(dist, cta->d_dists + cta->smem_storage.state.warp_comm[0][3]);
                    VertexId neighbor_id, tmp_neighbor_id;

                    while (coop_offset + KernelPolicy::THREADS < coop_oob)
                    {

                      // Gather
                      util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(tmp_neighbor_id, cta->d_column_indices + coop_offset + threadIdx.x);
//                      assert(tmp_neighbor_id >= -1 && tmp_neighbor_id < cta->nodes);
                      EValue nb_dist;
                      util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(nb_dist, cta->d_dists + tmp_neighbor_id);

//                      printf("CTA: bidx=%d, tidx=%d, vid=%d, dist=%d, neighbor_id=%d, nb_dist=%d\n", blockIdx.x, threadIdx.x, cta->smem_storage.state.warp_comm[0][3], dist, tmp_neighbor_id, nb_dist);
//                      if (changed == 1 && atomicCAS(cta->d_visited_flag + tmp_neighbor_id, 0, 1) == 0)
                      if ((changed == 1 || cta->iteration == 0) && dist + 1 < nb_dist)
                        neighbor_id = tmp_neighbor_id;
                      else
                        neighbor_id = -1;

//                      if (neighbor_id >= 6297539 || neighbor_id < -1) printf("SCAN: neighbor_id =%d\n", neighbor_id);
                      // Scatter neighbor
                      ...
                      util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(dist + 1, cta->d_predecessor_out + cta->smem_storage.state.coarse_enqueue_offset + coop_rank);
...
//                        VertexId predecessor_id = cta->smem_storage.state.warp_comm[warp_id][3];
                        int changed;		// = cta->smem_storage.state.warp_comm[warp_id][3];
                        util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(changed, cta->d_changed + cta->smem_storage.state.warp_comm[warp_id][3]);
                        EValue dist;
                        util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(dist, cta->d_dists + cta->smem_storage.state.warp_comm[warp_id][3]);

                        VertexId neighbor_id, tmp_neighbor_id;
                        while (coop_offset + B40C_WARP_THREADS_BFS(KernelPolicy::CUDA_ARCH) < coop_oob)
                        {
                          // Gather
                          util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(tmp_neighbor_id, cta->d_column_indices + coop_offset + lane_id);
//                          assert(tmp_neighbor_id >= -1 && tmp_neighbor_id < cta->nodes);
//                          if (changed == 1 && atomicCAS(cta->d_visited_flag + tmp_neighbor_id, 0, 1) == 0)
                          int nb_dist;
                          util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(nb_dist, cta->d_dists + tmp_neighbor_id);
//                          printf("WARP: bidx=%d, tidx=%d, vid=%d, dist=%d, neighbor_id=%d, nb_dist=%d\n", blockIdx.x, threadIdx.x, cta->smem_storage.state.warp_comm[warp_id][3], dist, tmp_neighbor_id, nb_dist);
                          if ((changed == 1 || cta->iteration == 0) && dist + 1 < nb_dist)
                          {
                            neighbor_id = tmp_neighbor_id;
                          }
...
//                          if (neighbor_id >= 6297539 || neighbor_id < -1) printf("WARP: neighbor_id =%d\n", neighbor_id);
                          // Scatter neighbor
                          util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(neighbor_id, cta->d_out + cta->smem_storage.state.coarse_enqueue_offset + coop_rank);

//                          printf("bidx=%d, tidx=%d, neighbor_id=%d\n", blockIdx.x, threadIdx.x, neighbor_id);
                          // Scatter predecessor
                          util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(dist + 1, cta->d_predecessor_out + cta->smem_storage.state.coarse_enqueue_offset + coop_rank);
...
//                          assert(tmp_neighbor_id >= -1 && tmp_neighbor_id < cta->nodes);
                          ...
                          int nb_dist;
                          util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(nb_dist, cta->d_dists + tmp_neighbor_id);
//                          printf("bidx=%d, tidx=%d, vid=%d, dist=%d, neighbor_id=%d, nb_dist=%d\n", blockIdx.x, threadIdx.x, cta->smem_storage.state.warp_comm[warp_id][3], dist, tmp_neighbor_id, nb_dist);
                          if ((changed == 1 || cta->iteration == 0) && dist + 1 < nb_dist)
                            neighbor_id = tmp_neighbor_id;
                          else
                            neighbor_id = -1;
...
//                  if (tile->vertex_id[LOAD][VEC] < 0 || tile->vertex_id[LOAD][VEC] > 8191)
//                  {
//                    printf("EXPAND: Error tile->vertex_id[LOAD][VEC]=%d\n", tile->vertex_id[LOAD][VEC]);
//                  }

                  int new_changed;
//                  util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(changed, cta->d_changed + tile->vertex_id[LOAD][VEC]);
                  EValue dist, nb_dist;
                  util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(dist, cta->d_dists + tile->vertex_id[LOAD][VEC]);
//                  util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(gather_result, cta->d_gather_results + tile->vertex_id[LOAD][VEC]);
                  util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(new_changed, cta->d_changed + tile->vertex_id[LOAD][VEC]);
//                  printf("bidx=%d, tidx=%d, vid=%d, dist=%d, changed=%d\n", blockIdx.x, threadIdx.x, tile->vertex_id[LOAD][VEC], dist, new_changed);
//                  EValue new_dist = min(dist, gather_result);
//                  if (dist != new_dist || cta->iteration == 0)
//                  {
//                    new_changed = 1;
//                    util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(new_dist, cta->d_dists_out + tile->vertex_id[LOAD][VEC]);
//                  }
//                  else
//                    new_changed = 0;
//                  if (tile->row_progress[LOAD][VEC] == 0) util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(changed - 2, cta->d_changed + tile->vertex_id[LOAD][VEC]);
...
//                    printf("SCAN: bidx=%d, tidx=%d, vid=%d, dist=%d, neighbor_id=%d, nb_dist=%d\n", blockIdx.x, threadIdx.x, tile->vertex_id[LOAD][VEC], dist, neighbor_id, nb_dist);

                    if ( (new_changed == 1 || cta->iteration == 0) && dist + 1 < nb_dist)
                    {
//                      if (cta->iteration == 2)
//                        printf("bidx=%d, tidx=%d, tile->row_offset[LOAD][VEC]=%d, tile->row_progress[LOAD][VEC]=%d\n", blockIdx.x, threadIdx.x, tile->row_offset[LOAD][VEC],
//                            tile->row_progress[LOAD][VEC]);

                      cta->smem_storage.gather_offsets[scratch_offset] = neighbor_id;                  //tile->row_offset[LOAD][VEC] + tile->row_progress[LOAD][VEC];
                      cta->smem_storage.gather_predecessors[scratch_offset] = dist + 1;//tile->vertex_id[LOAD][VEC];
                    }
                    else
                    {
                      cta->smem_storage.gather_offsets[scratch_offset] = -1;
                      cta->smem_storage.gather_predecessors[scratch_offset] = 100000000;
                    }
...
//                    tile->row_progress[LOAD][VEC] += tile->row_length[LOAD][VEC];
             ...
             * Constructor
             */
            __device__ __forceinline__ Cta(int iteration, SizeT nodes, SizeT edges, VertexId queue_index, int num_gpus, SmemStorage &smem_storage, VertexId *d_in, VertexId *d_out, VertexId *d_predecessor_out,
                VertexId *d_column_indices, SizeT *d_row_offsets, int* d_dists, int* d_dists_out, EValue* d_gather_results, int* d_changed, int* d_visited_flag, util::CtaWorkProgress &work_progress, SizeT max_edge_frontier) :

                iteration(iteration), nodes(nodes), edges(edges), queue_index(queue_index), num_gpus(num_gpus), smem_storage(smem_storage), raking_soa_details(
                    typename RakingSoaDetails::GridStorageSoa(smem_storage.coarse_raking_elements, smem_storage.fine_raking_elements),
                    typename RakingSoaDetails::WarpscanSoa(smem_storage.state.coarse_warpscan, smem_storage.state.fine_warpscan), TileTuple(0, 0)), d_in(d_in), d_out(d_out), d_predecessor_out(
                    d_predecessor_out), d_column_indices(d_column_indices), d_row_offsets(d_row_offsets), d_dists(d_dists), d_dists_out(d_dists_out), d_gather_results(d_gather_results), d_changed(d_changed), d_visited_flag(d_visited_flag), work_progress(
...
//                if (threadIdx.x == 0) printf("Expand_SCAN698: iteration=%d\n", iteration, SmemStorage::GATHER_ELEMENTS);
              tile.progress = 0;
...
//                printf("bidx=%d, tidx=%d, scratch_remainder=%d\n", blockIdx.x, threadIdx.x, scratch_remainder);

                for (int scratch_offset = threadIdx.x; scratch_offset < scratch_remainder; scratch_offset += KernelPolicy::THREADS)
                {
                  // Gather a neighbor
                  VertexId neighbor_id = smem_storage.gather_offsets[scratch_offset];
//                  assert(neighbor_id >= -1 && neighbor_id < nodes);

//                  if (neighbor_id >= 6297539 || neighbor_id < -2)
//                    printf("SCAN: neighbor_id =%d, scratch_offset=%d, gather_offsets=%d\n", neighbor_id, scratch_offset, smem_storage.gather_offsets[scratch_offset]);
                  ...
                  EValue predecessor_value = smem_storage.gather_predecessors[scratch_offset];
