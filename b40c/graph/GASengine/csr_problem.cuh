...
namespace b40c
{
  namespace graph
  {
    namespace GASengine
    {
       ...
       */
      template<typename _VertexId, typename _SizeT, typename _EValue, bool MARK_PREDECESSORS, 	// Whether to mark predecessors (vs. mark distance from source)
      ...
      struct CsrProblem
      {
        ...
        typedef ProblemType<_VertexId,				// VertexId
            _SizeT,					// SizeT
            _EValue,				// Edge Value
...
//          SizeT *d_visit_flags;          // Track if same vertex is being expanded inside the same frontier-queue
          ...
          GraphSlice(int gpu, cudaStream_t stream) :
              gpu(gpu), d_vertex_ids(NULL), d_column_indices(NULL), d_row_offsets(NULL), d_edge_values(NULL), d_node_values(NULL), d_dists(NULL), d_dists_out(NULL), d_gather_results(NULL), d_changed(
                  NULL), d_visited_flag(NULL), d_visited_mask(NULL), d_filter_mask(NULL), nodes(0), edges(0), stream(stream)
          {
...
//            if (d_deltas) util::B40CPerror(cudaFree(d_deltas), "GpuSlice cudaFree d_deltas failed", __FILE__, __LINE__);
            ...
            if (d_gather_results) util::B40CPerror(cudaFree(d_gather_results), "GpuSlice cudaFree d_gather_results failed", __FILE__, __LINE__);
            if (d_changed) util::B40CPerror(cudaFree(d_changed), "GpuSlice cudaFree d_changed failed", __FILE__, __LINE__);
            if (d_visited_flag) util::B40CPerror(cudaFree(d_visited_flag), "GpuSlice cudaFree d_dists_out failed", __FILE__, __LINE__);
...
//            if (d_visit_flags) util::B40CPerror(cudaFree(d_visit_flags), "GpuSlice cudaFree d_visit_flags failed", __FILE__, __LINE__);
         ...
         * all GPUs
         */
        cudaError_t ExtractResults(EValue *h_dists, VertexId *h_labels, EValue *h_sigmas, EValue *h_deltas)
        {
          ...
          do
          {
            if (graph_slices.size() == 1)
            {
              ...
              // Special case for only one GPU, which may be set as with
              // an ordinal other than 0.
              if (retval = util::B40CPerror(cudaMemcpy(h_dists, graph_slices[0]->d_dists, sizeof(EValue) * graph_slices[0]->nodes, cudaMemcpyDeviceToHost), "CsrProblem cudaMemcpy d_dists failed",
...
//              delete gpu_deltas;
        ...
        cudaError_t FromHostProblem(bool stream_from_host,			// Only meaningful for single-GPU BFS
            SizeT nodes, SizeT edges, VertexId *h_column_indices, SizeT *h_row_offsets, EValue *h_edge_values, VertexId *h_row_indices, SizeT *h_column_offsets, EValue *h_node_values, 
                                    int num_gpus, int device)
        {
          cudaError_t retval = cudaSuccess;
          this->nodes = nodes;
          this->edges = edges;
...
//              if (retval = util::B40CPerror(cudaGetDevice(&gpu), "CsrProblem cudaGetDevice failed", __FILE__, __LINE__)) break;
              ...
              else
              {
                ...
                if (retval = util::B40CPerror(cudaMalloc((void**) &graph_slices[0]->d_num_out_edges, (graph_slices[0]->nodes + 1) * sizeof(VertexId)), "CsrProblem cudaMalloc d_vertex_ids failed",
                    __FILE__, __LINE__)) break;
                ...
                util::MemsetKernel<EValue><<<memset_grid_size, memset_block_size, 0, graph_slices[0]->stream>>>(
...
//                thrust::device_ptr<int> d_row_offsets_ptr(graph_slices[0]->d_row_offsets);
//                thrust::device_ptr<int> d_num_out_edges_ptr(graph_slices[0]->d_num_out_edges);
//
//                thrust::adjacent_difference(thrust::device, d_row_offsets_ptr, d_row_offsets_ptr + graph_slices[0]->nodes + 1, d_num_out_edges_ptr);
//                graph_slices[0]->d_num_out_edges++;

//                VertexId *test_vid = new VertexId[graph_slices[0]->nodes];
//                cudaMemcpy(test_vid, graph_slices[0]->d_num_out_edges, graph_slices[0]->nodes * sizeof(VertexId), cudaMemcpyDeviceToHost);
//                printf("d_num_out_edges: ");
...
//            }
            ...
            if (!graph_slices[gpu]->d_dists)
            {
              ...
              printf("GPU %d d_dists: %lld elements (%lld bytes)\n", graph_slices[gpu]->gpu, (unsigned long long) graph_slices[gpu]->nodes,
                  (unsigned long long) graph_slices[gpu]->nodes * sizeof(int));

              if (retval = util::B40CPerror(cudaMalloc((void**) &graph_slices[gpu]->d_dists, graph_slices[gpu]->nodes * sizeof(EValue)), "CsrProblem cudaMalloc d_deltas failed", __FILE__, __LINE__))
                return retval;
            }

            if (!graph_slices[gpu]->d_dists_out)
            {

              printf("GPU %d d_dists_out: %lld elements (%lld bytes)\n", graph_slices[gpu]->gpu, (unsigned long long) graph_slices[gpu]->nodes,
                  (unsigned long long) graph_slices[gpu]->nodes * sizeof(int));

              if (retval = util::B40CPerror(cudaMalloc((void**) &graph_slices[gpu]->d_dists_out, graph_slices[gpu]->nodes * sizeof(EValue)), "CsrProblem cudaMalloc d_deltas failed", __FILE__,
                  __LINE__)) return retval;
            }

            if (!graph_slices[gpu]->d_gather_results)
            {

              printf("GPU %d d_gather_results: %lld elements (%lld bytes)\n", graph_slices[gpu]->gpu, (unsigned long long) graph_slices[gpu]->nodes,
                  (unsigned long long) graph_slices[gpu]->nodes * sizeof(int));

              if (retval = util::B40CPerror(cudaMalloc((void**) &graph_slices[gpu]->d_gather_results, graph_slices[gpu]->nodes * sizeof(EValue)), "CsrProblem cudaMalloc d_deltas failed", __FILE__,
                  __LINE__)) return retval;
            }
            ...
            if (!graph_slices[gpu]->d_visited_flag)
            {

              printf("GPU %d d_visited_flag: %lld elements (%lld bytes)\n", graph_slices[gpu]->gpu, (unsigned long long) graph_slices[gpu]->nodes,
                  (unsigned long long) graph_slices[gpu]->nodes * sizeof(int));

              if (retval = util::B40CPerror(cudaMalloc((void**) &graph_slices[gpu]->d_visited_flag, graph_slices[gpu]->nodes * sizeof(int)), "CsrProblem cudaMalloc d_deltas failed", __FILE__,
                  __LINE__)) return retval;
            }
...
//            }
            ...
            for (int i = 0; i < 3; i++)
            {
              ...
              if (graph_slices[gpu]->predecessor_elements[i] < new_predecessor_elements[i])
              {
                ...
                if (retval = util::B40CPerror(cudaMalloc((void**) &graph_slices[gpu]->frontier_queues.d_values[i], graph_slices[gpu]->predecessor_elements[i] * sizeof(VertexId)),
...
//             Initialize d_dists elements to 1.0
            memset_grid_size = B40C_MIN(memset_grid_size_max, (graph_slices[gpu]->nodes + memset_block_size - 1) / memset_block_size);
...
//            Initialize d_dists_out to d_dists
            if (retval = util::B40CPerror(cudaMemcpy(graph_slices[gpu]->d_dists_out, graph_slices[0]->d_dists, graph_slices[0]->nodes * sizeof(EValue), cudaMemcpyDeviceToDevice),
                "CsrProblem cudaMemcpy d_dists_out failed", __FILE__, __LINE__)) return retval;
//
//            if (retval = util::B40CPerror(cudaThreadSynchronize(), "MemsetKernel failed", __FILE__, __LINE__)) return retval;

//            if (retval = util::B40CPerror(cudaMemcpy(graph_slices[gpu]->d_dists, graph_slices[0]->d_vertex_ids, graph_slices[0]->nodes * sizeof(VertexId), cudaMemcpyDeviceToDevice),
//                "CsrProblem cudaMemcpy d_vertex_ids failed", __FILE__, __LINE__)) return retval;

            // Initialize d_gather_results elements to 0
            memset_grid_size = B40C_MIN(memset_grid_size_max, (graph_slices[gpu]->nodes + memset_block_size - 1) / memset_block_size);
            util::MemsetKernel<EValue><<<memset_grid_size, memset_block_size, 0, graph_slices[gpu]->stream>>>(
                graph_slices[gpu]->d_gather_results,
                100000000,
                graph_slices[gpu]->nodes);
            ...
            // Initialize d_visited_flag elements to 0
            memset_grid_size = B40C_MIN(memset_grid_size_max, (graph_slices[gpu]->nodes + memset_block_size - 1) / memset_block_size);
            util::MemsetKernel<int><<<memset_grid_size, memset_block_size, 0, graph_slices[gpu]->stream>>>(
                graph_slices[gpu]->d_visited_flag,
                0,
                graph_slices[gpu]->nodes);
            if (retval = util::B40CPerror(cudaThreadSynchronize(), "MemsetKernel failed", __FILE__, __LINE__)) return retval;
