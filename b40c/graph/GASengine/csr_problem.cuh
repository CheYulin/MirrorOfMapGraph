...
template<typename _Program, typename _VertexId, typename _SizeT,
		...
		bool WITH_VALUE> // Whether to include edge/ndoe value computation with BFS
struct CsrProblem {
	//---------------------------------------------------------------------
...
//          VertexType gather_list_out;
		...
		GraphSlice(int gpu, cudaStream_t stream) :
				gpu(gpu), d_vertex_ids(NULL), d_column_indices(NULL), d_row_offsets(
						NULL), d_edge_values(NULL), d_node_values(NULL), d_labels(
						NULL), d_preds(NULL), d_sigmas(NULL), d_deltas(NULL), d_dists(
						NULL), d_changed(NULL), d_visited_mask(NULL), d_filter_mask(
						NULL), d_visit_flags(NULL), nodes(0), edges(0), stream(
						stream) {
		...
		virtual ~GraphSlice() {
			...
			// Free pointers
			if (d_vertex_ids)
				util::B40CPerror(cudaFree(d_vertex_ids),
						"GpuSlice cudaFree d_vertex_ids failed", __FILE__,
						__LINE__);
			if (d_column_indices)
			...
			if (d_node_values)
				util::B40CPerror(cudaFree(d_node_values),
						"GpuSlice cudaFree d_node_values", __FILE__, __LINE__);
...
//          }
	 ...
	 * all GPUs
	 */
	cudaError_t ExtractResults(VertexId *h_dists, VertexId *h_labels,
			EValue *h_sigmas, EValue *h_deltas) {
		...
		do {
			if (graph_slices.size() == 1) {
				...
				if (retval = util::B40CPerror(
						cudaMemcpy(h_dists,
								graph_slices[0]->vertex_list.d_dists,
...
//              delete gpu_deltas;
	...
	cudaError_t FromHostProblem(
			...
			SizeT nodes, SizeT edges, VertexId *h_column_indices,
			SizeT *h_row_offsets, EValue *h_edge_values,
			VertexId *h_row_indices, SizeT *h_column_offsets,
			EValue *h_node_values, int num_gpus) {
...
//              graph_slices[0]->gather_list.size = nodes;
				...
				if (stream_from_host) {
					...
					if (retval =
							util::B40CPerror(
									...
									"CsrProblem cudaHostGetDevicePointer d_column_indices failed",
									__FILE__, __LINE__))
						break;
					...
					if (retval =
							util::B40CPerror(
									...
									"CsrProblem cudaHostGetDevicePointer d_row_offsets failed",
									__FILE__, __LINE__))
						break;
					...
					if (retval =
							util::B40CPerror(
									...
									"CsrProblem cudaHostGetDevicePointer d_edge_values failed",
									__FILE__, __LINE__))
						break;
					...
					if (retval =
							util::B40CPerror(
									cudaHostGetDevicePointer(
											...
											(void *) h_node_values, 0),
									"CsrProblem cudaHostGetDevicePointer d_node_values failed",
									__FILE__, __LINE__))
						break;
					...
					printf(
							"GPU %d column_indices: %lld elements (%lld bytes)\n",
							graph_slices[0]->gpu,
					...
					if (retval = util::B40CPerror(
							cudaMalloc((void**) &graph_slices[0]->d_vertex_ids,
									graph_slices[0]->nodes * sizeof(VertexId)),
							"CsrProblem cudaMalloc d_vertex_ids failed",
							__FILE__, __LINE__))
						break;
					...
					if (retval = util::B40CPerror(
							cudaMemcpy(graph_slices[0]->d_row_indices,
									h_row_indices,
									graph_slices[0]->edges * sizeof(VertexId),
									cudaMemcpyHostToDevice),
							"CsrProblem cudaMemcpy d_row_indices failed",
							__FILE__, __LINE__))
						break;

					if (retval = util::B40CPerror(
							cudaMemcpy(graph_slices[0]->d_vertex_ids,
									h_vertex_ids,
									graph_slices[0]->nodes * sizeof(VertexId),
									cudaMemcpyHostToDevice),
							"CsrProblem cudaMemcpy d_vertex_ids failed",
							__FILE__, __LINE__))
						break;
					...
					if (retval = util::B40CPerror(
							cudaMalloc((void**) &graph_slices[0]->d_edge_values,
									graph_slices[0]->edges * sizeof(VertexId)),
							"CsrProblem cudaMalloc d_edge_values failed",
							__FILE__, __LINE__))
						break;
					...
					if (retval = util::B40CPerror(
							cudaMalloc((void**) &graph_slices[0]->d_row_offsets,
									(graph_slices[0]->nodes + 1)
											* sizeof(SizeT)),
							"CsrProblem cudaMalloc d_row_offsets failed",
							__FILE__, __LINE__))
						break;
					...
					if (retval = util::B40CPerror(
							cudaMemcpy(graph_slices[0]->d_row_offsets,
									...
									(graph_slices[0]->nodes + 1)
											* sizeof(SizeT),
									cudaMemcpyHostToDevice),
							"CsrProblem cudaMemcpy d_row_offsets failed",
							__FILE__, __LINE__))
						break;
					...
					if (retval = util::B40CPerror(
							cudaMemcpy(graph_slices[0]->d_column_offsets,
									...
									(graph_slices[0]->nodes + 1)
											* sizeof(SizeT),
									cudaMemcpyHostToDevice),
							"CsrProblem cudaMemcpy d_column_offsets failed",
							__FILE__, __LINE__))
						break;

					if (retval = util::B40CPerror(
							cudaMalloc((void**) &graph_slices[0]->d_node_values,
									(graph_slices[0]->nodes) * sizeof(EValue)),
							"CsrProblem cudaMalloc d_node_values failed",
							__FILE__, __LINE__))
						break;
					...
					util::MemsetKernel<EValue><<<memset_grid_size,
							memset_block_size, 0, graph_slices[0]->stream>>>(
							graph_slices[0]->d_node_values, 0,
				...
				for (int gpu = 0; gpu < num_gpus; gpu++) {
					...
					// Set device
					if (retval = util::B40CPerror(cudaSetDevice(gpu),
							"CsrProblem cudaSetDevice failed", __FILE__,
							__LINE__))
						break;
					...
					if (retval = util::B40CPerror(cudaStreamCreate(&stream),
							"CsrProblem cudaStreamCreate failed", __FILE__,
							__LINE__))
						break;
				...
				for (VertexId node = 0; node < nodes; node++) {
					...
					graph_slices[gpu]->edges += h_row_offsets[node + 1]
							- h_row_offsets[node];
				}
				...
				for (int gpu = 0; gpu < num_gpus; gpu++) {
					...
					printf("GPU %d gets %lld vertices and %lld edges\n", gpu,
							(long long) graph_slices[gpu]->nodes,
							(long long) graph_slices[gpu]->edges);
					fflush (stdout);

					slice_row_offsets[gpu] = new SizeT[graph_slices[gpu]->nodes
							+ 1];
					slice_row_offsets[gpu][0] = 0;

					slice_vertex_ids[gpu] =
							new VertexId[graph_slices[gpu]->nodes];
					slice_column_indices[gpu] =
							new VertexId[graph_slices[gpu]->edges];
					slice_edge_values[gpu] =
							new EValue[graph_slices[gpu]->edges];
					slice_node_values[gpu] =
							new EValue[graph_slices[gpu]->nodes];
				...
				for (VertexId node = 0; node < nodes; node++) {
					...
					int gpu = GpuIndex(node);
					VertexId slice_row = GraphSliceRow(node);
					SizeT row_edges = h_row_offsets[node + 1]
							- h_row_offsets[node];

					memcpy(
							slice_column_indices[gpu]
									+ slice_row_offsets[gpu][slice_row],
					...
					memcpy(
							slice_vertex_ids[gpu]
									+ slice_row_offsets[gpu][slice_row],
							h_vertex_ids + h_row_offsets[node],
							row_edges * sizeof(VertexId));
					...
					slice_row_offsets[gpu][slice_row + 1] =
							graph_slices[gpu]->edges;

					// Mask in owning gpu
					for (int edge = 0; edge < row_edges; edge++) {
						VertexId *ptr = slice_column_indices[gpu]
								+ slice_row_offsets[gpu][slice_row] + edge;
				...
				for (int gpu = 0; gpu < num_gpus; gpu++) {
					...
					if (retval = util::B40CPerror(
							cudaMemcpy(graph_slices[gpu]->d_row_offsets,
					...
					if (retval = util::B40CPerror(
							cudaMemcpy(graph_slices[gpu]->d_column_indices,
									slice_column_indices[gpu],
					...
					if (retval = util::B40CPerror(
							cudaMalloc(
									(void**) &graph_slices[gpu]->d_vertex_ids,
									graph_slices[gpu]->edges
											* sizeof(VertexId)),
							"CsrProblem cudaMalloc d_vertex_ids failed",
							__FILE__, __LINE__))
					...
					if (retval = util::B40CPerror(
							cudaMemcpy(graph_slices[gpu]->d_vertex_ids,
									slice_vertex_ids[gpu],
									graph_slices[gpu]->nodes * sizeof(VertexId),
									cudaMemcpyHostToDevice),
							"CsrProblem cudaMemcpy dverte_ids failed", __FILE__,
							__LINE__))
						break;
					...
					if (WITH_VALUE) {
						if (retval = util::B40CPerror(
								cudaMemcpy(graph_slices[gpu]->d_edge_values,
										slice_edge_values[gpu],
										graph_slices[gpu]->edges
												* sizeof(EValue),
										cudaMemcpyHostToDevice),
							...
							break;
						if (retval = util::B40CPerror(
								cudaMemcpy(graph_slices[gpu]->d_node_values,
										slice_node_values[gpu],
										graph_slices[gpu]->nodes
												* sizeof(EValue),
										cudaMemcpyHostToDevice),
								"CsrProblem cudaMemcpy d_node_values failed",
								__FILE__, __LINE__))
							break;
					}
					// Cleanup host construction arrays
					if (slice_row_offsets[gpu])
						delete slice_row_offsets[gpu];
					if (slice_vertex_ids[gpu])
						delete slice_vertex_ids[gpu];
					if (slice_column_indices[gpu])
						delete slice_column_indices[gpu];
					if (slice_edge_values[gpu])
						delete slice_edge_values[gpu];
					if (slice_node_values[gpu])
						delete slice_node_values[gpu];
				}
				if (retval)
					break;
				...
				if (slice_row_offsets)
					delete slice_row_offsets;
				if (slice_vertex_ids)
					delete slice_vertex_ids;
				if (slice_column_indices)
					delete slice_column_indices;
				if (slice_edge_values)
					delete slice_edge_values;
				if (slice_node_values)
					delete slice_node_values;
			}
...
//            graph_slices[gpu]->gather_list.init();
			...
			if (!graph_slices[gpu]->d_labels) {
				...
				if (retval = util::B40CPerror(
						cudaMalloc((void**) &graph_slices[gpu]->d_labels,
								graph_slices[gpu]->nodes * sizeof(VertexId)),
						"CsrProblem cudaMalloc d_labels failed", __FILE__,
						__LINE__))
					return retval;
			}
			...
			if (!graph_slices[gpu]->d_preds) {
				...
				if (retval = util::B40CPerror(
						cudaMalloc((void**) &graph_slices[gpu]->d_preds,
								graph_slices[gpu]->nodes * sizeof(VertexId)),
						"CsrProblem cudaMalloc d_preds failed", __FILE__,
						__LINE__))
					return retval;
			}
			...
			if (!graph_slices[gpu]->d_sigmas) {
				...
				if (retval = util::B40CPerror(
						cudaMalloc((void**) &graph_slices[gpu]->d_sigmas,
								graph_slices[gpu]->nodes * sizeof(EValue)),
						"CsrProblem cudaMalloc d_sigmas failed", __FILE__,
						__LINE__))
					return retval;
			}
			...
			if (!graph_slices[gpu]->d_deltas) {
				...
				if (retval = util::B40CPerror(
						cudaMalloc((void**) &graph_slices[gpu]->d_deltas,
								graph_slices[gpu]->nodes * sizeof(EValue)),
						"CsrProblem cudaMalloc d_deltas failed", __FILE__,
						__LINE__))
					return retval;
			}
			...
			if (!graph_slices[gpu]->d_dists) {
				...
				if (retval = util::B40CPerror(
						cudaMalloc((void**) &graph_slices[gpu]->d_dists,
								graph_slices[gpu]->nodes * sizeof(int)),
						"CsrProblem cudaMalloc d_deltas failed", __FILE__,
						__LINE__))
					return retval;
			}
			...
			if (!graph_slices[gpu]->d_changed) {
				...
				if (retval = util::B40CPerror(
						cudaMalloc((void**) &graph_slices[gpu]->d_changed,
								graph_slices[gpu]->nodes * sizeof(int)),
						"CsrProblem cudaMalloc d_deltas failed", __FILE__,
						__LINE__))
					return retval;
			...
			if (!graph_slices[gpu]->d_visit_flags) {
				...
				if (retval = util::B40CPerror(
						cudaMalloc((void**) &graph_slices[gpu]->d_visit_flags,
								graph_slices[gpu]->nodes * sizeof(SizeT)),
						"CsrProblem cudaMalloc d_visit_flags failed", __FILE__,
						__LINE__))
					return retval;
			...
			for (int i = 0; i < 3; i++) {
				...
				if (graph_slices[gpu]->frontier_elements[i]
					...
					printf(
							"GPU %d frontier queue[%d] (queue-sizing factor %.2fx): %lld elements (%lld bytes)\n",
				...
				if (graph_slices[gpu]->predecessor_elements[i]
					...
					printf(
							"GPU %d predecessor queue[%d] (queue-sizing factor %.2fx): %lld elements (%lld bytes)\n",
							graph_slices[gpu]->gpu, i, queue_sizing,
