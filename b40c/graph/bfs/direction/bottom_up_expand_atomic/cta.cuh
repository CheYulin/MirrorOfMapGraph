...
/******************************************************************************
 * CTA tile-processing abstraction for bottom-up BFS frontier expansion
 ******************************************************************************/
...
namespace bottom_up_expand_atomic {
    ...
    template <typename KernelPolicy>
        struct Cta
        {
            ...
            bool     				*d_frontier_in;				// Input frontier bitmap
            bool                    *d_frontier_out;            // Output frontier bitmap
                                        ...
                                        static __device__ __forceinline__ void ExpandByCta(Cta *cta, Tile *tile)
                                        {
                                            ...
                                            while (true) {
                                                ...
                                                if (owner == threadIdx.x) {
                                                    ...
                                                    cta->smem_storage.state.warp_comm[0][3] = tile->vertex_id[LOAD][VEC];									    // current vertex
                                                ...
                                                VertexId vertex_id;
                                                vertex_id = cta->smem_storage.state.warp_comm[0][3];
                                                /*util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(
                                                            false,
                                                            cta->d_frontier_out + vertex_id);
                                                util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(
                                                            -1,
                                                            cta->d_out + vertex_id);
                                                ...
                                                if (KernelPolicy::MARK_PREDECESSORS) {
                                                        ...
                                                        util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(
                                                                -1,
                                                                cta->d_predecessor_out + vertex_id);
                                                    }*/
                                                __syncthreads();
                                                ...
                                                while (coop_offset + KernelPolicy::THREADS < coop_oob) {
                                                    ...
                                                    util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(
                                                            in_frontier, cta->d_frontier_in + parent_id);

                                                    if (in_frontier) {
                                                        // Scatter bitmap value
                                                        /*util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(
                                                            true,
                                                            cta->d_frontier_out + vertex_id);
                                                        util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(
                                            ...
                                            if (KernelPolicy::WARP_GATHER_THRESHOLD < KernelPolicy::CTA_GATHER_THRESHOLD) {
                                                ...
                                                while (__any(tile->row_length[LOAD][VEC] >= KernelPolicy::WARP_GATHER_THRESHOLD)) {
                                                    ...
                                                    if (lane_id == cta->smem_storage.state.warp_comm[warp_id][0]) {
                                                        ...
                                                        cta->smem_storage.state.warp_comm[warp_id][3] = tile->vertex_id[LOAD][VEC];								        // current vertex
                                                    ...
                                                    VertexId vertex_id;
                                                    vertex_id = cta->smem_storage.state.warp_comm[warp_id][3];
                                                    /*util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(
                                                            false,
                                                            cta->d_frontier_out + vertex_id);
                                                    util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(
                                    ...
                                    template <typename Cta, typename Tile>
                                        static __device__ __forceinline__ void ExpandByScan(Cta *cta, Tile *tile)
                                        {
                                                    ...
                                                    (scratch_offset < SmemStorage::GATHER_ELEMENTS))
                                            {
                                                ...
                                                VertexId vertex_id = cta->smem_storage.gather_predecessors[scratch_offset];
                                                /*util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(
                                                    false,
                                                    cta->d_frontier_out + vertex_id);
                                                util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(
            ...
            __device__ __forceinline__ Cta(
                ...
                d_in(d_in),
                d_out(d_out),
                d_frontier_in(d_frontier_in),
                d_frontier_out(d_frontier_out),
                    ...
                    SizeT guarded_elements = KernelPolicy::TILE_ELEMENTS)
            {
                ...
                while (tile.progress < tile.fine_count) {
                            ...
                            scratch_offset += KernelPolicy::THREADS)
                    {
                        ...
                        bool in_frontier;
                        util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(
                                parent_id, d_column_indices + smem_storage.gather_offsets[scratch_offset]);
                        util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(
                                in_frontier, d_frontier_in + parent_id);

                        VertexId vertex_id = smem_storage.gather_predecessors[scratch_offset];

                        /*if (in_frontier) {
                            // Scatter bitmap value
                            util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(
                                true,
                                d_frontier_out + vertex_id);
                            util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(
                                vertex_id,
                                d_out + vertex_id);
                            if (KernelPolicy::MARK_PREDECESSORS) {
