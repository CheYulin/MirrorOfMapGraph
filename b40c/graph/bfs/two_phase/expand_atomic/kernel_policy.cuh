	...
	struct SmemStorage
	{
		...
		enum {
			...
			SCRATCH_ELEMENT_SIZE 			= //(ProblemType::MARK_PREDECESSORS) ?
													sizeof(SizeT) + sizeof(VertexId) + sizeof(EValue),			// Need both gather offset and predecessor
													//sizeof(SizeT) + sizeof(EValue),								// Just gather offset
		...
		union {
			...
			struct {
                ...
                EValue                      gather_vertex_values[GATHER_ELEMENTS];
			};
		};
