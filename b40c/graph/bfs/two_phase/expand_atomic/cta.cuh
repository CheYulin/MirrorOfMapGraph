...
namespace expand_atomic {
    ...
    template <typename KernelPolicy>
        struct Cta
        {
            ...
            EValue                  *d_edge_values;             // Edge values array
                                    ...
                                    template <typename Cta, typename Tile>
                                        static __device__ __forceinline__ void ExpandByCta(Cta *cta, Tile *tile)
                                        {
                                            ...
                                            while (true) {
                                                ...
                                                cta->smem_storage.gather_vertex_values[threadIdx.x] = 0; // reset scratchpad for partial vertex result
                                                ...
                                                while (coop_offset + KernelPolicy::THREADS < coop_oob) {
                                                    ...
                                                    if (KernelPolicy::WITH_VALUE)
                                                    {
                                                        // Gather edge value
                                                        util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(
                                                                edge_value, cta->d_edge_values + coop_offset + threadIdx.x);

                                                        cta->smem_storage.gather_vertex_values[threadIdx.x] += edge_value;
                                                    }
                                            ...
                                            if (KernelPolicy::WITH_VALUE)
                                            {
                                                EValue vertex_value = CTAReduce<EValue>(cta->smem_storage.gather_vertex_values);
                                                // vie for write into vertex_value
                                                if (threadIdx.x == 0)
                                                {
                                                    ...
                                                    {
                                                        //safe to write into d_node_values
                                                        util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(
                                                                vertex_value, cta->d_node_values+tile->vertex_id[LOAD][VEC]);
                                                    }
                                                }
                                            }
                                            // Next vector element
                                        ...
                                        static __device__ __forceinline__ void ExpandByWarp(Cta *cta, Tile *tile)
                                        {
                                            if (KernelPolicy::WARP_GATHER_THRESHOLD < KernelPolicy::CTA_GATHER_THRESHOLD) {
                                                ...
                                                while (__any(tile->row_length[LOAD][VEC] >= KernelPolicy::WARP_GATHER_THRESHOLD)) {
                                                    ...
                                                    cta->smem_storage.gather_vertex_values[warp_id*B40C_WARP_THREADS_BFS(KernelPolicy::CUDA_ARCH)+lane_id] = 0; //reset scratchpad
                                                    ...
                                                    while (coop_offset  + B40C_WARP_THREADS_BFS(KernelPolicy::CUDA_ARCH) < coop_oob) {
                                                        ...
                                                        if (KernelPolicy::WITH_VALUE)
                                                        {
                                                            // Gather edge value
                                                            util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(
                                                                    edge_value, cta->d_edge_values + coop_offset + lane_id);

                                                            cta->smem_storage.gather_vertex_values[warp_id*B40C_WARP_THREADS_BFS(KernelPolicy::CUDA_ARCH)+lane_id] += edge_value;
                                                        }
                                                    ...
                                                    if (KernelPolicy::WITH_VALUE)
                                                    {
                                                        EValue vertex_value = WarpReduce(cta->smem_storage.gather_vertex_values, warp_id);
                                                        // vie for write into vertex_value
                                                        if (threadIdx.x == 0)
                                                        {
                                                            //try to set the d_visit_flag in global memory
                                    ...
                                    template <typename Cta, typename Tile>
                                        static __device__ __forceinline__ void ExpandByScan(Cta *cta, Tile *tile)
                                        {
                                                    ...
                                                    (scratch_offset < SmemStorage::GATHER_ELEMENTS))
                                            {
                                                ...
                                                //Need predecessor to specify which vertex's neighbor list values are we adding up.
                                                ...
                                                if (KernelPolicy::WITH_VALUE)
                                                {
                                                    util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(
                                                            edge_value,
                                                            cta->d_edge_values + cta->smem_storage.gather_offsets[scratch_offset]);
                                                    vertex_value += edge_value; 
                                                }
                                            ...
                                            if (KernelPolicy::WITH_VALUE)
                                            {
                                                ...
                                                if (visit_flag == 0)
                                                {
                                                    //first write, directly write into d_node_values
                                                    util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(
                                                            vertex_value, cta->d_node_values+tile->vertex_id[LOAD][VEC]);
                                                ...
                                                {
                                                    //second write, last time interrupted by scratchpad size full
                                                    //need to load and add then write back
                                                    util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(
                                                            edge_value,
                                                            cta->d_node_values+tile->vertex_id[LOAD][VEC]);
                                                    vertex_value += edge_value;
                                                    util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(
                                                            vertex_value, cta->d_node_values+tile->vertex_id[LOAD][VEC]);
