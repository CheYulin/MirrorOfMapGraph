		...
		do {
			if (graph_slices.size() == 1) {
                ...
                if (retval = util::B40CPerror(cudaMemcpy(
                        ...
                        cudaMemcpyDeviceToHost),
                    "CsrProblem cudaMemcpy d_edge_value failed", __FILE__, __LINE__)) break;
		...
		do {
			if (num_gpus <= 1) {
				...
				if (stream_from_host) {
                    ...
                    SizeT *h_visit_flag = new SizeT[graph_slices[gpu]->nodes];
                    for (int i = 0; i < graph_slices[gpu]->nodes; ++i)
                    {
                	    h_visit_flag[i] = 0;
                    }
                    if (retval = util::B40CPerror(cudaMemcpy(
						    graph_slices[0]->d_visit_flags,
						    h_visit_flag,
						    sizeof(SizeT) * graph_slices[0]->nodes,
						    cudaMemcpyHostToDevice),
					    "CsrProblem cudaMemcpy d_visit_flags failed", __FILE__, __LINE__)) break;
				    delete[] h_visit_flag;
