...
struct CsrProblem
{
	 ...
	 * all GPUs
	 */
	cudaError_t ExtractResults(VertexId *h_label, EValue *h_node_values, EValue *h_sigmas, EValue *h_deltas)
	{
		...
		do {
			if (graph_slices.size() == 1) {
                ...
                if (retval = util::B40CPerror(cudaMemcpy(
                        h_sigmas,
                        graph_slices[0]->d_sigmas,
                        sizeof(EValue) * graph_slices[0]->nodes,
                        cudaMemcpyDeviceToHost),
                    "CsrProblem cudaMemcpy d_sigmas failed", __FILE__, __LINE__)) break;

                //copy delta values out from GPU
                if (retval = util::B40CPerror(cudaMemcpy(
                        h_deltas,
                        graph_slices[0]->d_deltas,
                        sizeof(EValue) * graph_slices[0]->nodes,
                        cudaMemcpyDeviceToHost),
                    "CsrProblem cudaMemcpy d_deltas failed", __FILE__, __LINE__)) break;
				...
				for (int gpu = 0; gpu < num_gpus; gpu++) {
                    ...
                    if (retval = util::B40CPerror(cudaMemcpy(
                            gpu_node_values[gpu],
                            graph_slices[gpu]->d_node_values,
                            sizeof(EValue) * graph_slices[0]->nodes,
                            cudaMemcpyDeviceToHost),
		...
		do {
			if (num_gpus <= 1) {
				...
				if (stream_from_host) {
                    ...
                    if (retval = util::B40CPerror(cudaMalloc(
							(void**) &graph_slices[0]->d_vertex_ids,
							graph_slices[0]->nodes * sizeof(VertexId)),
						"CsrProblem cudaMalloc d_vertex_ids failed", __FILE__, __LINE__)) break;
					...
					if (retval = util::B40CPerror(cudaMalloc(
							(void**) &graph_slices[0]->d_edge_values,
							graph_slices[0]->edges * sizeof(VertexId)),
						"CsrProblem cudaMalloc d_edge_values failed", __FILE__, __LINE__)) break;
                    ...
                    if (retval = util::B40CPerror(cudaMalloc(
						...
						"CsrProblem cudaMalloc d_node_values failed", __FILE__, __LINE__)) break;
				...
				for (VertexId node = 0; node < nodes; node++) {
                	...
                	memcpy(
						slice_vertex_ids[gpu] + slice_row_offsets[gpu][slice_row],
            ...
            if (retval = util::B40CPerror(cudaMalloc(
                    ...
                    if (WITH_VALUE)
                    {
                       if (retval = util::B40CPerror(cudaMemcpy(
                                ...
                                graph_slices[gpu]->edges * sizeof(EValue),
                                cudaMemcpyHostToDevice),
                            "CsrProblem cudaMemcpy d_edge_values failed", __FILE__, __LINE__)) break;
                        if (retval = util::B40CPerror(cudaMemcpy(
                                graph_slices[gpu]->d_node_values,
		...
		double queue_sizing)			// Size scaling factor for work queue allocation (e.g., 1.0 creates n-element and m-element vertex and edge frontiers, respectively).  0.0 is unspecified.
	{
		...
		for (int gpu = 0; gpu < num_gpus; gpu++) {
            ...
            if (!graph_slices[gpu]->d_preds) {
				...
				if (retval = util::B40CPerror(cudaMalloc(
						(void**) &graph_slices[gpu]->d_preds,
						graph_slices[gpu]->nodes * sizeof(VertexId)),
					"CsrProblem cudaMalloc d_preds failed", __FILE__, __LINE__)) return retval;
			}
            ...
            if (!graph_slices[gpu]->d_sigmas) {
				...
				if (retval = util::B40CPerror(cudaMalloc(
						(void**) &graph_slices[gpu]->d_sigmas,
						graph_slices[gpu]->nodes * sizeof(EValue)),
					"CsrProblem cudaMalloc d_sigmas failed", __FILE__, __LINE__)) return retval;
			}
            ...
            if (!graph_slices[gpu]->d_deltas) {
				...
				if (retval = util::B40CPerror(cudaMalloc(
						(void**) &graph_slices[gpu]->d_deltas,
						graph_slices[gpu]->nodes * sizeof(EValue)),
					"CsrProblem cudaMalloc d_deltas failed", __FILE__, __LINE__)) return retval;
			}
            ...
            if (!graph_slices[gpu]->d_visit_flags) {
                ...
                if (retval = util::B40CPerror(cudaMalloc(
                            ...
                            "CsrProblem cudaMalloc d_visit_flags failed", __FILE__, __LINE__)) return retval;
