      ...
      namespace vertex_centric
      {
        namespace backward_sum_atomic
        {
          ...
          struct Cta
          {
              ...
              struct Iterate
              {
                ...
                static __device__ __forceinline__ void ExpandByCta(Cta *cta, Tile *tile)
                {
                  ...
                  while (true)
                  {
                    ...
                    EValue result;
                    util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(from_sigma_value, cta->d_sigmas + tile->vertex_id[LOAD][VEC]);
                    while (coop_offset + KernelPolicy::THREADS < coop_oob)
                    {
                      ...
                      util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(to_sigma_value, cta->d_sigmas + neighbor_id);
                      util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(delta_value, cta->d_deltas + neighbor_id);
                  ...
                  if (threadIdx.x == 0)
                  {
                    if (atomicCAS(cta->d_visit_flags + tile->vertex_id[LOAD][VEC], 0, 1) == 0)
                    {
                      ...
                      util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(current_delta, cta->d_node_values + tile->vertex_id[LOAD][VEC]);
                      current_delta += final_delta_value;
                      util::io::ModifiedStore<KernelPolicy::QUEUE_WRITE_MODIFIER>::St(current_delta, cta->d_node_values + tile->vertex_id[LOAD][VEC]);
                    }
                  }
                ...
                static __device__ __forceinline__ void ExpandByScan(Cta *cta, Tile *tile)
                {
                  ...
                  while ((tile->row_progress[LOAD][VEC] < tile->row_length[LOAD][VEC]) && (scratch_offset < SmemStorage::GATHER_ELEMENTS))
                  {
                    ...
                    util::io::ModifiedLoad<KernelPolicy::COLUMN_READ_MODIFIER>::Ld(dst_depth, cta->d_labels + neighbor_id);
             ...
             * Constructor
             */
            __device__ __forceinline__ Cta(int iteration, VertexId queue_index, int num_gpus, SmemStorage &smem_storage, VertexId *d_in, VertexId *d_column_indices, SizeT *d_row_offsets,
                EValue *d_node_values, VertexId *d_labels, EValue *d_sigmas, EValue *d_deltas, VertexId *d_visit_flags, util::CtaWorkProgress &work_progress, SizeT max_edge_frontier) :

                iteration(iteration), queue_index(queue_index), num_gpus(num_gpus), smem_storage(smem_storage), raking_soa_details(
                    typename RakingSoaDetails::GridStorageSoa(smem_storage.coarse_raking_elements, smem_storage.fine_raking_elements),
                    typename RakingSoaDetails::WarpscanSoa(smem_storage.state.coarse_warpscan, smem_storage.state.fine_warpscan), TileTuple(0, 0)), d_in(d_in), d_column_indices(d_column_indices), d_row_offsets(
                    d_row_offsets), d_node_values(d_node_values), d_labels(d_labels), d_sigmas(d_sigmas), d_deltas(d_deltas), d_visit_flags(d_visit_flags), work_progress(work_progress), max_edge_frontier(
                    max_edge_frontier)
            {
              if (threadIdx.x == 0)
              {
