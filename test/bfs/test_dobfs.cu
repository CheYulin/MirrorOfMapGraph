...
/******************************************************************************
 * Simple test driver program for Direction Optimal BFS graph traversal.
 *
 ******************************************************************************/
...
// BFS includes
#include <b40c/graph/bfs/csr_csc_problem.cuh>
...
#include <b40c/graph/bfs/enactor_direction_optimal.cuh>
...
bool g_with_value;          // Whether or not to load value from graph data
...
void Usage() 
{
	printf("\ntest_dobfs <graph type> <graph type args> [--device=<device index>] "
            ...
            "  fromfile <file>\n"
            "    Reads graph data from file. Default source vertex is random.\n"
			"\n"
			...
			"\n"
			"--with-value Whether or not include simple edge/node computation in BFS. \n"
			"\n"
	...
	Stats									&stats)								// running statistics
{
	...
	search_depth++;
	printf("finished simple reference\n");
...
{
	typedef bfs::CsrCscProblem<VertexId, SizeT, Value, MARK_PREDECESSORS, WITH_VALUE> CsrCscProblem;
	...
	bfs::EnactorDirection<INSTRUMENT>		direction_optimal(g_verbose);

	// Allocate Stats map
	std::map<Strategy, Stats*> stats_map;
	stats_map[HOST] 				= new Stats("Simple CPU BFS");
	stats_map[DIRECTION_OPTIMAL] 	= new Stats("Direction-optimal GPU BFS");

	// Allocate problem on GPU
	CsrCscProblem csr_csc_problem;
	if (csr_csc_problem.FromHostProblem(
		g_stream_from_host,
		csr_graph.nodes,
		csr_graph.edges,
	...
	while (test_iteration <= test_iterations) {
			...
			++itr)
		{
			...
			switch (strategy) {
			case DIRECTION_OPTIMAL:
				if (retval = csr_csc_problem.Reset(direction_optimal.GetFrontierType(), max_queue_sizing)) break;
				gpu_timer.Start();
				if (retval = direction_optimal.EnactSearch(csr_csc_problem, src, max_grid_size)) break;				// Iterative variant
				gpu_timer.Stop();
				direction_optimal.GetStatistics(total_queued, search_depth, avg_duty);
				break;
			}
			...
			// Copy out results
			if (csr_csc_problem.ExtractResults(h_labels, h_node_values)) exit(1);
			...
			{
				DisplayStats<CsrCscProblem::ProblemType::MARK_PREDECESSORS>(
					*stats,
					src,
					h_labels,
					reference_check,
	...
	CommandLineArgs &args)
{
	...
	std::vector<int> strategies(1, DIRECTION_OPTIMAL);			// Use "direction optimal" strategy by default
...
int main( int argc, char** argv)
{
	...
	if (graph_type == "grid2d") {
        ...
        // Directed graph built from file
