...
#include <b40c/graph/builder/charitynet.cuh>
...
{
	typedef bfs::CsrProblem<VertexId, SizeT, Value, MARK_PREDECESSORS, WITH_VALUE> CsrProblem;
	...
	while (test_iteration <= test_iterations) {
			...
			++itr)
		{
			...
			// Copy out results
			if (csr_problem.ExtractResults(h_labels, h_node_values)) exit(1);
	...
	CommandLineArgs &args)
{
	...
	std::vector<int> strategies(1, TWO_PHASE);			// Use "two phase" strategy by default
...
int main( int argc, char** argv)
{
	...
	if (graph_type == "grid2d") {
       ...
       if (builder::BuildCharityNetGraph<true>(filename, csr_graph) != 0) {
           return 1;
       }
       printf("start bfs.\n");
