...
ZHI_BIN=/home/zhisong/Dropbox/systap/XDATA/mpgraph-code/branches/BFS
DATA_DIR=/home/zhisong/Dropbox/GraphDatasets
...
GEN_DATA="bitcoin webbase-1M wikipedia-20070206 twitter_d1 traceroute"
...
#do
    for graph in $GRAPHS
	  ...
	  #echo "graph=[$graph], directedGraph=[$directedGraph]"
          for src in ${vertices}
          do
	   cmd="$exec $DATA_DIR/$graph$suffix foo $src $undirectedGraph 2 3.0"
           echo "Running: $cmd"
           $cmd >> zhi.$type-nogather.$graph.log 2>&1
          done
       done
#done
