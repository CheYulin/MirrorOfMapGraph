  ...
  struct scatter {
    __host__ __device__
      int operator()(VertexType &dst, VertexType &src, int &e) {
        ...
        if(src.dist == dst.dist-1 && dst.changed)
        {
...
struct adaptiveBC_backward {
  ...
  struct gather {
    __host__ __device__
      ...
      if(flag && dst.dist == src.dist+1 && src.dist > 0)
      {
       return (double)src.sigma / dst.sigma * (1.0 + dst.BC);
      }
      else 
        return 0.0;
      }
