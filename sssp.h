...
struct sssp {
	...
	static void Initialize(const int nodes, const int edges, int num_srcs, int* srcs, VertexType &vertex_list, int* d_frontier_keys[3], MiscType* d_frontier_values[3])
	{
		vertex_list.nodes = nodes;
		vertex_list.edges = edges;

		b40c::util::B40CPerror(cudaMalloc((void**) &vertex_list.d_dists, nodes * sizeof(DataType)), "cudaMalloc VertexType::d_dists failed", __FILE__, __LINE__);
		b40c::util::B40CPerror(cudaMalloc((void**) &vertex_list.d_dists_out, nodes * sizeof(DataType)), "cudaMalloc VertexType::d_dists_out failed", __FILE__, __LINE__);
		b40c::util::B40CPerror(cudaMalloc((void**) &vertex_list.d_changed, nodes * sizeof(int)), "cudaMalloc VertexType::d_changed failed", __FILE__, __LINE__);
		b40c::util::B40CPerror(cudaMalloc((void**) &vertex_list.d_min_dists, nodes * sizeof(DataType)), "cudaMalloc VertexType::d_min_dists failed", __FILE__, __LINE__);
		...
		if (b40c::util::B40CPerror(
				cudaMemcpy(vertex_list.d_dists,
						init_dists,
						num_srcs * sizeof(int),
		...
		if (b40c::util::B40CPerror(
				cudaMemcpy(d_frontier_keys[0],
						srcs,
						num_srcs * sizeof(int),
						cudaMemcpyHostToDevice),
				"CsrProblem cudaMemcpy d_frontier_keys failed", __FILE__, __LINE__))
			exit(0);

		if (b40c::util::B40CPerror(
				cudaMemcpy(d_frontier_keys[1],
						srcs,
						num_srcs * sizeof(int),
						cudaMemcpyHostToDevice),
				"CsrProblem cudaMemcpy d_frontier_keys failed", __FILE__, __LINE__))
		...
		if (b40c::util::B40CPerror(
				cudaMemcpy(d_frontier_values[0],
						init_value,
						num_srcs * sizeof(int),
						cudaMemcpyHostToDevice),
				"CsrProblem cudaMemcpy d_frontier_values failed", __FILE__, __LINE__))
			exit(0);

		if (b40c::util::B40CPerror(
						cudaMemcpy(d_frontier_values[1],
								init_value,
								num_srcs * sizeof(int),
								cudaMemcpyHostToDevice),
						"CsrProblem cudaMemcpy d_frontier_values failed", __FILE__, __LINE__))
		...
		if (b40c::util::B40CPerror(
				cudaMemcpy(vertex_list.d_changed,
						init_changed,
						num_srcs * sizeof(int),
						cudaMemcpyHostToDevice),
	   ...
	   */
		  struct gather_vertex {
			__device__
			  void operator()(const int row_id, const GatherType final_value, VertexType &vertex_list) {
	  ...
	  struct gather_edge {
		__device__
		  void operator()(const int row_id, const int neighbor_id_in, const VertexType &vertex_list,  GatherType& new_value) {
	 ...
	 */
	  struct gather_sum {
		__device__
		GatherType operator()(const GatherType &left, const GatherType &right) {
			  return left + right;
		  }
	  };
  ...
  /** Update the vertex state given the gather results. */
	  struct apply {
		 ...
		 * 
		 */
		  void operator()(const int vertex_id, const int iteration, VertexType& vertex_list) {
  ...
  */
	  struct expand_edge {
		   ...
		   */
		  void operator()(const bool changed, const int iteration, const int vertex_id, const int neighbor_id_in, VertexType& vertex_list, int& frontier, int& misc_value)
		  {
   ...
   */
	  struct contract {
		   ...
		   * function.
		   */
		  void operator()(const int iteration, int &vertex_id, VertexType &vertex_list,  int& misc_value)
		  {
