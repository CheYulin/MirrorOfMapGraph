...
//      exit(0);
    ...
    memset_grid_size =
        B40C_MIN(memset_grid_size_max, (num_srcs + memset_block_size - 1) / memset_block_size);
    b40c::util::MemsetKernel<DataType><<<memset_grid_size,
    memset_block_size, 0, 0>>>(d_frontier_values[0], 0,
        num_srcs);

    memset_grid_size =
            B40C_MIN(memset_grid_size_max, (num_srcs + memset_block_size - 1) / memset_block_size);
        b40c::util::MemsetKernel<DataType><<<memset_grid_size,
        memset_block_size, 0, 0>>>(d_frontier_values[1], 0,
            num_srcs);
...
//      printf("vertex_id=%d, edge_id=%d, neighbor_id_in=%d, edge_value=%d\n", vertex_id, edge_id, neighbor_id_in, edge_value);
      if ((changed) && dst_dist > src_dist + edge_value)
