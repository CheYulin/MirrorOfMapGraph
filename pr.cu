...
//  double max_queue_sizing = 1.3;
	...
	for (int i = 1; i < argc; i++) {
		...
		else if (strncmp(argv[i], "-PR", 100) == 0) { //The PR specific options
			i++;
			cfg.parseParameterString(argv[i]);
		} else if (strncmp(argv[i], "-c", 100) == 0) { //use a configuration file to specify the PR options instead of command line
			i++;
